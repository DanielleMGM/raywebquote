...
stockid:0,customid:0,kitID:0,primaryVoltage:0,primaryVoltageDW:"",secondaryVoltage:0,secondaryVoltageDW:"",electrostaticShield:"SH",taps:0,customCatalogNumber:"",quantity:0,updateQuantity:false},b),success:a,error:c,cache:false})},update:function(b,a,c,d){$.ajax({url:"Ajax.aspx?action=QuoteDetailCreate",type:"get",dataType:"json",contentType:"application/json; charset=utf-8",data:{quoteid:a.quoteID,stockid:a.stockUnitID,customid:a.customStockID,kitID:a.kitID,primaryVoltage:a.customPrimaryVoltage,
primaryVoltageDW:a.customPrimaryVoltageDW,secondaryVoltage:a.customSecondaryVoltage,secondaryVoltageDW:a.customSecondaryVoltageDW,electrostaticShield:a.electrostaticShield,taps:a.taps,customCatalogNumber:a.customCatalogNumber,quantity:a.quantity,updateQuantity:true},success:c,error:d,cache:false})},destroy:function(b,a,c){return $.ajax({url:"Ajax.aspx?action=QuoteDetailDelete&quoteDetailId="+b+"&quoteID="+this.attributes.quoteID,type:"get",dataType:"json",success:a,error:c,cache:false})}},{formattedprimaryvoltage:function(){return this.formattedvoltage(this.customPrimaryVoltage,
this.customPrimaryVoltageDW)},formattedsecondaryvoltage:function(){return this.formattedvoltage(this.customSecondaryVoltage,this.customSecondaryVoltageDW)},formattedvoltage:function(b,a){return a=="Y"?b+"Y/"+Math.round(parseInt(b)/Math.sqrt(3)):b+" D"}});
;
steal.end();
...
$(a).siblings("select");$(a).val()=="Y"?$.each(b.find("option"),function(c,d){c=$(d).text().replace(" D","");var e="";if(c=="")return true;e=$.trim(c)+"Y/"+Math.round(parseInt(c)/Math.sqrt(3));$(d).text(e)}):$.each(b.find("option"),function(c,d){c=$(d).text().split("/");var e=c[0].replace("Y"," D");if(c=="")return true;$(d).text(e)})},"#shield-type input[type=radio] change":function(){this.getcustomcatalog()},getcustomcatalog:function(){if(this.valid("#special-unit",true))Mgmtransformer.Models.Customcatalog.findOne({winding:this.$specialunit.find("li#special-windings input:checked").val(),
...
seshield:this.$specialunit.find('input[name="special-electrostatic-shield"]:checked').val(),taps:this.$specialunit.find("#special-taps").val(),kit:this.$specialunit.find("#special-kit-quantity").val()>0?this.$specialunit.find("#specialkitID").val():0},this.callback("showcustomcatalog"));else{this.resetcustomcatalog();return false}},showcustomcatalog:function(a){this.$specialunit.find("#special-catalogNumber").html("<span>"+a.catalogNumber+"</span>").end().find("#specialID").val(a.customID).end();
var b='<span class="consult">Consult factory for pricing</span>';if(a.price!="0"){b="$"+a.price;this.$specialunit.find("#special-quantity").val(0).removeAttr("disabled").removeClass("disabled").end()}else this.$specialunit.find("#special-quantity").val(0).attr("disabled","disabled").addClass("disabled").end();this.$specialunit.find("#special-price").html("<span>"+b+"</span>").end();if(a.kit!=""){this.$specialunit.find("#specialkitID").val(a.kitID).end().find("#special-kit-price").html("<span> $"+
a.kitPrice+"</span>").end().find("#special-kit-quantity").removeAttr("disabled").removeClass("disabled").end().find("#specialnone").hide().end().find("#specialwall").hide().end().find("#specialrain").hide().end();if(a.kit=="WB 1"||a.kit=="WB 2")this.$specialunit.find("#specialwall").show();else if(a.kit=="RH 5"||a.kit=="RH 6")this.$specialunit.find("#specialrain").show();this.$specialunit.find("#specialkitID").val(a.kitID).end().find("#specialkkit").css({visibility:"visible"})}else this.resetcustomkit(a.price!=
"0")},resetcustomseletion:function(){this.$specialunit.find("#special-quantity").val("0").end().find("#special-kva").val("-1").end().find("#special-temp").val("-1").end().find("#special-kfactor").val("-1").end().find("#special-sound").val("-1").end().find("#special-primary-voltage").val("-1").end().find("#special-primary-delta").click().end().find("#special-secondary-voltage").val("-1").end().find("#special-electrostatic-shield").click().end().find("#special-taps").val("-1").end().find("#special-windings-aluminum").click().end().find("#special-secondary-delta").click().end();
...
electrostaticShield:this.$specialunit.find('input[name="special-electrostatic-shield"]:checked').val(),taps:this.$specialunit.find("#special-taps").val(),customCatalogNumber:this.$specialunit.find("#special-catalogNumber").text(),kitID:this.$specialunit.find("#special-kit-quantity").val()>0?this.$specialunit.find("#specialkitID").val():0})).save(b)},addspecialaccessory:function(a){$("#quoteID").val(a.quoteid);(new Mgmtransformer.Models.Quotedetail({quoteid:$("#quoteID").val(),kitID:this.$specialunit.find("#specialkitID").val(),
quantity:this.$specialunit.find("#special-kit-quantity").val()})).save(this.callback("updatecustomquoteid"))},updatecustomquoteid:function(a){$("#quoteID").val(a.quoteid);this.resetcustomseletion();this.clearerror();this.confirmation(this.$specialunit.find("#special-add"));this.hideloader($("#newquote"))},"#btn-gotoquote click":function(){$("#quoteID").val()!=0?this.element.trigger("showdetail",[$("#quoteID").val(),false]):alert("The cart is empty")}});
;
steal.end();
...
___v1ew.push("  Enter a New Quote\n");
___v1ew.push("</h2>\n");
...
___v1ew.push("        <option value=\"");___v1ew.push((jQuery.EJS.text( Voltage[i].Voltage)));___v1ew.push("\">");___v1ew.push((jQuery.EJS.text( Voltage[i].Voltage)));___v1ew.push(" D\n");
___v1ew.push("        </option>\n");
___v1ew.push("        ");};___v1ew.push("</select>\n");
...
___v1ew.push("    </li>\n");
___v1ew.push("    <li>\n");
___v1ew.push("      <label for=\"special-electrostatic-shield\" class=\"master\">\n");
___v1ew.push("        Electrostatic Shield\n");
___v1ew.push("      </label>\n");
___v1ew.push("      <input name=\"special-electrostatic-shield\" id=\"special-electrostatic-none\" class=\"shield-type\" type=\"radio\" value=\"0\"/>\n");
___v1ew.push("      <label for=\"special-electrostatic-none\">\n");
___v1ew.push("        None\n");
___v1ew.push("      </label>\n");
___v1ew.push("      <input name=\"special-electrostatic-shield\" id=\"special-electrostatic-one\" class=\"shield-type\" type=\"radio\" value=\"SH\" checked=\"checked\" />\n");
___v1ew.push("      <label for=\"special-electrostatic-one\">\n");
___v1ew.push("        One\n");
___v1ew.push("      </label>\n");
___v1ew.push("    </li>\n");
___v1ew.push("    <li class='clear'>\n");
...
___v1ew.push("        <option value=\"3\">2-2½% FCAN and 4-2½% FCBN (Standard)</option>\n");
___v1ew.push("      </select>\n");
___v1ew.push("    </li>\n");
___v1ew.push("  </ul>\n");
___v1ew.push("\n");
